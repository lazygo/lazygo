...
func SendAdminNotify(email string, subject string, content string) error {
	builder := &strings.Builder{}
	data := map[string]string{
		"subject": SmtpTpl["admin"] + "：" + subject,
		"content": content,
	}
	err := adminTpl.Execute(builder, data)
	if err != nil {
		return err
	}
	return mail.MailTo(email, "Lazygo——管理员通知", builder.String())
}

func init() {
	adminTpl, _ = template.New("admin-email").Parse(adminHtml)
}
    ...
    <td>
      ...
      <div style="width:680px;padding:0 10px;margin:0 auto;">
        <div style="line-height:1.5;font-size:14px;margin-bottom:25px;color:#4d4d4d;">
          <strong style="display:block;margin-bottom:15px;">尊敬的Lazygo管理员：<span
