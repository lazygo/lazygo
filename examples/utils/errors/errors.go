...
var (
	...
	ErrPasswordError        = server.NewHTTPError(200, 10005, "密码长度不得小于6位")
	ErrPasswordTooLongError = server.NewHTTPError(200, 10006, "密码长度太长")
	ErrUsernameError        = server.NewHTTPError(200, 10007, "仅支持使用邮箱或手机号登录")

	ErrUploadCosFail      = server.NewHTTPError(200, 10101, "文件上传服务异常")
	ErrInvalidImageFormat = server.NewHTTPError(200, 10102, "图片格式错误")
)
